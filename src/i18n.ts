...
const messages = {
  ...
  openFailed: { 'zh-CN': '打开失败：文件可能已损坏或格式不受支持', 'en-US': 'Open failed: the file may be damaged or unsupported', 'zh-TW': '開啟失敗：檔案可能已損壞或格式不受支援' },
  saveFailed: { 'zh-CN': '保存失败：请检查文件权限和磁盘空间', 'en-US': 'Save failed: check file permissions and disk space', 'zh-TW': '儲存失敗：請檢查檔案權限與磁碟空間' },
  importFailed: { 'zh-CN': '导入失败：文件可能已损坏或格式不受支持', 'en-US': 'Import failed: the file may be damaged or unsupported', 'zh-TW': '匯入失敗：檔案可能已損壞或格式不受支援' },
  exportFailed: { 'zh-CN': '导出失败：请检查目标位置和磁盘空间', 'en-US': 'Export failed: check the destination and disk space', 'zh-TW': '匯出失敗：請檢查目標位置與磁碟空間' },
